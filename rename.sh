...
echo "Renaming all 'patches_api' -> '$SNAKE'"
...
# folder
find . -type d -not -path '*/\.git/*' | sed -e "p;s/patches_api/$SNAKE/" | xargs -n2 mv
...
# file names
find . -type f -not -path '*/\.git/*' | sed -e "p;s/patches_api/$SNAKE/" | xargs -n2 mv
...
# strings
find . -type f -not -path '*/\.git/*' -exec sed -i "s/patches_api/$SNAKE/g" {} \;
